...
------------------
  ...
  4.  Color System
      4a. Semantic Color Roles (locked)
      4b. Background & Surface Palette
      4c. Colors Under Consideration (pending visual test)
      4d. Rejected Options
  5.  Typography Scale
  6.  Spacing System
  7.  Alignment Standards
  8.  Button Design Standards
  9.  Component Standards
  10. Interaction Patterns
  11. UI Ideas Brainstorm
...
-----------------------------
  ...
  LIGHT / DARK MODE
    ...
    Light mode: post-MVP (needs a full light palette before implementing)
...
----------------
  ...
  4b. BACKGROUND & SURFACE PALETTE (dark mode — locked)
  -------------------------------------------------------
  Warm undertone applied throughout. Supports nature theme.
    ...
    Green accent: dark mode #3BAF6A / light mode #2D9F58 — locked (OD1). See section 4c.

  4c. GREEN ACCENT — LOCKED (OD1)
  ----------------------------------
  ...
  Candidates and analysis preserved below for reference.

  GREEN ACCENT — CANDIDATE B: #3BAF6A  "Leafy Mid"
  ...
  GREEN ACCENT — CANDIDATE C: #2D9F58  "Deeper Leafy"
  ...
  SURFACE WARM TINT — LOCKED (applied in 4b)
  ...
  LIGHT MODE PALETTE (post-MVP reference — lock values now, implement later)
  ...
  TWO-ACCENT SYSTEM (green + gold — confirmed in principle, palette pending)
...
Tagged by implementation status:
  ...
  PROFILE / SETTINGS
    ...
    [REQUIRES]  Light mode toggle — needs full light palette; post-MVP
...
Resolve before building the affected component.
  ...
  OD6 — CSS button standards / design token system
        ...
        Task: Review section 8 (Button Design Standards) and decide how to encode these
        rules in CSS — e.g. CSS custom properties in :root (--btn-height, --btn-font-size,
        etc.), shared utility classes in index.css, or another approach. Goal: future
        buttons reference variables instead of hardcoded values so consistency is
  ...
  OD1 — Accent color
        ...
        Rationale: brighter green has better contrast on dark surfaces; darker green has better contrast on white
...
13. DECISIONS LOCKED FROM THIS DOCUMENT
-----------------------------------------
