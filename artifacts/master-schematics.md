...
| modifications | `string[]` | Multi-select. Default `[]`. Forward-compat (UX deferred — F39). Conventional values: `paused` / `tempo` / `partial` / `lengthened` / `peak-contraction` / `explosive` / `deficit`. **Renamed from `bias` and reshaped to multi-select per Decision #32 (Session 52).** The word "bias" is reserved for a future field capturing muscle-loading emphasis as a result of a modification (deferred to next dev cycle). |
...
- **back:** lats, upperBack, lowerBack, lowerTraps
- **shoulders:** frontDelts, sideDelts, rearDelts, upperTraps, rotatorCuff, neck
...
> `upperTraps` lives in **shoulders** (Decision #30, Session 52). Bodybuilding shoulder-day convention; scapular elevation/shrug kinematics align with delts more than spinal stabilizers. `lowerTraps` stays in back (scap retraction + depression with rhomboids/mid traps). Tag assignments on individual exercises are unaffected — only the broad-group derivation moved.
...
- 29 pre-seeded exercises — re-curated for CE1 with full-schema tagging: `primaryMuscles`, role-tagged `secondaryMuscles`, plus Tier 3 forward-compat fields (`equipment`, `gripWidth`, `gripOrientation`, `stanceWidth`, `modifications`, `jointLoad`) populated where accurate data is available; otherwise null / `[]`
...
- **Save** → `ExerciseService.create({ name, primaryMuscles, secondaryMuscles, parentExerciseId, equipment: null, gripWidth: null, gripOrientation: null, stanceWidth: null, modifications: [], jointLoad: [], isCustom: true })` → new exercise immediately selected via `onSelect(created)` → modal closes (parent component unmounts on `onSelect`).
...
| 30 | `upperTraps` group derivation: back → shoulders (Session 52) | Bodybuilding shoulder-day convention; scapular elevation/shrug kinematics align with delts more than spinal stabilizers. `lowerTraps` stays in back (scap retraction + depression with rhomboids/mid traps — anchors with the rest of the back). Tag assignments on individual exercises unchanged — only `MUSCLE_TO_GROUP['upperTraps']` flipped from `'back'` to `'shoulders'`. Effects: picker Step 2 muscle-chip section, Back/Shoulders chip filter inclusion, and any future stats roll-up by group (F27/F24). No re-curation required. |
| 31 | Farmer Carry + Suitcase Carry primary order swap (Session 52) | Sequel to #30. Both carries' `primaryMuscles` reordered from `['forearms', 'upperTraps']` → `['upperTraps', 'forearms']`. Effect: `getExerciseGroup()` now returns `shoulders` instead of `arms` for both, routing them to the Shoulders chip alongside shrugs. Rationale: heavy carries are functionally a shrug-for-time isometric — upper trap stimulus dominates the training response; forearm grip is the limiting factor but not the primary tissue adaptation. Co-primary status (both muscles tagged primary) unchanged — only order swapped. Other carries (Overhead Carry: `frontDelts` → shoulders ✅; Zercher Carry: `upperBack, biceps` → back ✅) unaffected. **Curator note:** Suitcase Carry currently has `obliques` only as synergist; should arguably be co-primary (anti-lateral flexion is its differentiating stimulus vs Farmer). Flagged for a future tagging audit pass — not in scope for #31. |
| 32 | `bias` → `modifications` rename + multi-select reshape (Session 52) | Schema field `Exercise.bias: string \| null` renamed to `Exercise.modifications: string[]` (default `[]`). Multi-select: a single exercise can carry multiple modification tags (e.g., a Paused Deficit RDL could be `['paused', 'deficit']`). Value enum extended: existing 6 (`paused` / `tempo` / `partial` / `lengthened` / `peak-contraction` / `explosive`) + new `deficit` = 7. **The word "bias" is reserved for a future field** capturing which muscle takes more load as a result of a modification (e.g., Deficit RDL biases `hamstrings` within its `[glutes, hamstrings]` co-primary). Shape + UX of the future `bias` field are deferred to next dev cycle (rolls into F39 — Group 2 dimensions). **Code-only changes:** `types/index.ts` field rename + shape; `seed.ts` Tier3 alias + helper default + 11 entries (each single-string value wrapped as one-element array); `ExerciseSearchModal.tsx` custom-create payload; 6 test fixture files. **Pre-launch safety:** no DB migration needed — Decision #28 nukes-and-reseeds on v3 hop, renamed property repopulates from fresh seed run. Verified: tsc clean, build clean, 130/130 tests. |
...
| F32 | Future | Feature toggle menu (D-new) — granular per-feature opt-ins for advanced tracking dimensions (RPE, modifications, fatigue ratio, grip, stance, tempo, bilateral). Default minimal experience; power users opt in to dimensions they want. Absorbs `users.rpeEnabled` (currently surfaced standalone in Profile — Decision #26). See `memory/project_feature_toggle_menu.md`. (`bias` reserved for a future field capturing muscle-loading emphasis — see Decision #32.) | Post-MVP (next cycle) | Profile, App-wide | Future |
...
| F38 | Future | Cardio / conditioning tracking — doesn't fit current muscle-target schema; needs separate log shape. Slot under future exercise type/equipment dimension (F39 — D15 next cycle). See `memory/project_cardio_tracking.md`. **Session 53 #11 dependency lock:** cardio resolution waits on per-category tracking-options + corresponding toggle switches being built first (F32 toggle menu + F39 Group 2 dimensions + a category-tied tracking-shape architecture). Each exercise category (strength / cardio / plyometric / etc.) needs its own log shape + opt-in toggles before cardio's specific shape can be locked. Do NOT ship cardio in isolation — it's the second consumer of a multi-shape tracking architecture, not a standalone build. | Post-MVP (dedicated cycle) | Logs, DB | Future |
| F39 | Future | Group 2 dimensions (D10–D17) — full UX + feature work for modifications, fatigue ratio, grip width, grip orientation, stance width, equipment, tempo, bilateral. Tier 3 schema fields already live in v3 (`exercises.equipment`, `gripWidth`, `gripOrientation`, `stanceWidth` as nullable strings; `modifications` as `string[]`) — no future schema bump needed. UX, enums, consumer logic, and per-dimension-specific decisions (e.g. D15.2–D15.5 load input UX, D16 tempo definition-level vs program-level vs per-set, D17 logSets.side for bilateral) deferred to next cycle(s). **Note:** the field `modifications` was renamed from `bias` per Decision #32; the word `bias` is reserved for a future field capturing muscle-loading emphasis as a result of a modification (e.g., Deficit RDL biases hamstrings within `[glutes, hamstrings]` co-primary) — that future field is also part of F39 scope. | Post-MVP (next cycle) | Exercises, Logs, Stats | Future |
| F40 | Future | Picker filter revamp — multi-axis (raised + UX direction locked Session 52). **UX layout LOCKED:** Row A (always visible) = 6 muscle group chips (current); Row B (always visible) = mixed equipment + exercise-type chips (`barbell`, `dumbbell`, `machine`, `cable`, `bodyweight`, `kettlebell`, `band`, `cardio`, `olympic lift`, `plyometric`, etc.) — multi-select within row; Row C (conditional) = specific muscles for the currently-selected group (e.g. tap "Legs" → shows quads/hamstrings/glutes/calves chips below). Row C appears only when a single group is active; hides when "All" is selected. **Still parked:** data-model — Path X = single combined "category" column flattens type+equipment (UX-simple, loses orthogonality); Path Y = two independent columns (`exerciseType` + existing `equipment`) (cleaner schema, same flat UI). **Schema status:** `exercises.equipment` already exists in v3 (Tier 3 nullable string — populated per `seed-tagging-principles.md` "Always populate"); `exerciseType` / movement-category does NOT exist and would need a v4 schema bump under Path Y, or could be derived under Path X. See `memory/project_picker_ux_post_build.md` § "Multi-axis filter (equipment + exercise type)" for full discussion. Prerequisites: F38 (cardio data model), F39 (Group 2 dimensions — covers equipment UX/enums). **v4 bundling:** if Path Y wins, bundle the `exerciseType` add with F41's `aliases: string[]` add into a single v4 schema hop. **Session 53 #10 Row C lock-ins (4):** (1) **State cleanup** — whenever `activeGroup` changes (switched to a different group OR cleared to null), `selectedSubMuscles` clears automatically; never carry stale Row C state across group changes. (2) **Filter semantics — any-primary match** — Row C filter is `selectedSubMuscles.some(m => exercise.primaryMuscles.includes(m))`, NOT `primaryMuscles[0]`-only. Reason: RDL has `['glutes', 'hamstrings']`; tapping Hamstrings in Row C must surface RDL even though glutes is first. Side effect (acceptable): same exercise can appear under multiple sub-muscle filters. (3) **Conditional Row C visibility** — only render Row C when `MUSCLES_IN_GROUP[activeGroup].length >= 2`. Single-sub-muscle groups (none today, but future-proof) skip Row C entirely. (4) **UX cost flag** — on 480px width, 3 chip rows stack (Row A 6 chips + Row B ~10 chips + Row C up to 7 chips for Legs) above search + list. F40 designer must plan for collapse-on-scroll, smaller chips, horizontal scroll, or row-priority hide rules. Not a logic flaw, a real layout cost. **Closes Session 53 punch-list #10** (legs split into Quads/Hams/Glutes). Path A (true taxonomy split touching `MuscleGroup` union + stats roll-ups) explicitly rejected — Path B (drill-down chips, no taxonomy change) chosen. | Post-MVP (Phase 6+) | Exercises, DB | Future |
| F41 | Future | Per-exercise search aliases (locked Session 52). Add `aliases: string[]` field to `Exercise` (v4 schema bump, default `[]` on existing rows). `ExerciseService.search` scans `name` + `aliases` for matches. **Why this approach:** scales as the library grows pre- and post-launch — every new entry (including user customs) comes pre-wired with its own jargon; no central synonym file to maintain. Aliases travel with the row (export/sync friendly), and "why does 'fly' match this?" → check the entry, not a separate file. Rejected: global synonym map — easier short-term, painful at scale because every new entry forces a central-file edit. **Closes-by:** Session 53 punch-list #4 (Pec Deck → `["fly"]`), #5 (Cable Crossover → `["fly", "cable fly"]`), #14 (Romanian Deadlift → `["RDL"]`), #15 (Staggered-Stance RDL → `["b-stance", "b stance"]` — preserves searchability after the rename). **Build steps:** (1) v4 schema migration adding `aliases: string[]` default `[]`; (2) seed alias pass — light, only obvious jargon (Pec Deck, Cable Crossover, RDL, Smith Machine Squat, etc., not exhaustive); (3) `ExerciseService.search` updated to scan name + aliases; (4) optional: "alternate names" input on Step 1 of custom-create flow (defer if context tight — defaults to `[]`). **v4 bundling candidate:** combine with F40's `exerciseType` (if Path Y wins) into one v4 hop. | Post-MVP (Phase 6+) | Exercises, DB | Future |
| F42 | Future | Bodyweight + added-resistance redesign (Session 53 #1 deferred). Scope: pull-ups, dips, chin-ups, push-up family, plus select plyometric variants where weighted progression is common (e.g. Clap Push-Up + vest, Box Jump + vest). Need a separate "added resistance" field on those exercises so stats can compute honest volume = bodyweight + added load × reps. **Rejected at Session 53 triage** (all three patches insufficient): (A) label-only — kicks the can, stats stays dishonest; (B) total-display + edit-only-the-add — depends on `users.weight` being current, silently shifts old logs when bodyweight changes; (C) two fields per row — crams the SetRow grid (just retuned Session 52). Right answer is a dedicated redesign cycle, not a patch. **Likely shape:** flag affected exercises in the library (new `loadModel: 'bodyweight-plus-added' \| 'machine-assist' \| ...` enum or similar), add `LogSet.addedWeight: number \| null` (v4 schema), snapshot `LogSet.userBodyweight` at write time so historical totals don't drift. Machine-assist (Machine Assisted Pull-Up / Dip) likely shares mechanism with negative addedWeight. UI work non-trivial: SetRow + ExerciseCard column-headers need re-templating; conditional render based on the load-model flag. **v4 bundling candidate:** combine with F40 (`exerciseType`, if Path Y wins) + F41 (`aliases`) into a single v4 hop. See `memory/project_bodyweight_added_resistance.md`. | Post-MVP (Phase 6+) | Exercises, Logs, DB, Stats | Future |
...
| CP2 | Cleanup | `exercise-bank.md` group-label drift audit (Session 53 finding). Two rows had stale group labels relative to `muscleTaxonomy.ts` MUSCLE_TO_GROUP — now corrected post-Decisions #30 (upperTraps → shoulders) + #31 (carry primary order swap routes Farmer/Suitcase to Shoulders). Other rows likely have similar drift; needs a sweep across the full bank. Trigger: when next touching exercise-bank.md for any reason, run the audit at the same time. | Post-MVP (opportunistic) | Exercises, Docs | Pending |
| CP3 | Cleanup | `seed-tagging-principles.md` mid-trap clarification note (Session 53 finding). Mid-trap function is captured implicitly via `upperBack` — there is no separate `midTraps` taxon in `Muscle` union. Without an explicit note, future curators may incorrectly tag rows with `upperTraps` when meaning mid-traps (e.g. row, face pull, scap retraction work). Action: add a one-line clarification under Group conventions (Back / Shoulders sections) stating "mid-trap function lives under `upperBack`; do NOT use `upperTraps` for mid-trap involvement — that ID is for the shrug/elevation function only (post-#30, in shoulders group)." Trigger: next principles-doc edit OR next library curation pass. | Post-MVP (opportunistic) | Exercises, Docs | Pending |
...
| F8 | Future | Bodyweight + added weight display — `isBodyweight` boolean patch for "BW +X lbs" display | Superseded by F42 (Session 53) — full bodyweight + added-resistance redesign with `LogSet.addedWeight` + `userBodyweight` snapshot covers display AND stats honesty; F8's narrower scope fully absorbed |
...
| Date | Change |
|------|--------|
| 2026-04-26 | **Session 52 — Decision #32 locked: `bias` → `modifications` rename + multi-select reshape.** Schema field renamed and reshaped from `bias: string \| null` to `modifications: string[]` (default `[]`). Multi-select; value enum extended with `deficit` (7 total). The word "bias" reserved for a future field capturing muscle-loading emphasis (deferred to next cycle, rolls into F39). Code (10 files) + spec (master-schematics, seed-tagging-principles, seed-draft, exercise-bank, project_ce1_final_scope, CLAUDE.md, recap, handoff) all updated. Verification: tsc clean, build clean, 130/130 tests. No DB migration needed — pre-launch v3 nuke-and-reseed (Decision #28). |
| 2026-04-26 | **Session 52 close — partial Step 7. Tests + SetRow stepper rebuild + F40/F41 logged.** Vitest coverage shipped: `ExerciseSearchModal.test.tsx` 26 cases (130/130 across 11 files). SetRow rebuilt with custom `[− input +]` pill stepper per cell (native browser spinner hidden via `appearance: textfield` + `-webkit-appearance: none`). Grid retuned: BEST 44px, LB `minmax(72px, 1fr)`, REPS 56px, RPE 76px, gap 2px, buttons 18×36. Headers in `ExerciseCard.module.css` synced to same template — labels stay over inputs. Save logic split into `saveWeight` / `saveReps` / `saveRpe` so a blank field never fail-validates an adjacent field. Closes Session 53 punch-list #2 (weight clipping at 3+ digits) and #3 (negative cap — buttons clamp at 0). F40 expanded with v4 bundling note (exerciseType + aliases in one hop). F41 added: per-exercise `aliases: string[]` (v4 schema bump) — closes-by absorbs Session 53 punch-list #4/#5/#14. Step 7 manual smoke + Issue Tracker scrub deferred to Session 53 alongside the remaining ~14 punch-list items. |
| 2026-04-26 | **Session 52 — Decision #31 locked: Farmer Carry + Suitcase Carry primary order swap.** Sequel to #30. Both carries' `primaryMuscles` reordered to `['upperTraps', 'forearms']` so `getExerciseGroup()` routes them to Shoulders chip alongside shrugs. Heavy-carry trap stimulus dominates over forearm grip in training adaptation. Co-primary tagging unchanged — only order swapped. Other carries (Overhead, Zercher) unaffected. Curator flag added to #31 row: Suitcase Carry obliques should arguably be co-primary (deferred to future tagging audit). |
| 2026-04-26 | **Session 52 — Decision #30 locked: `upperTraps` group derivation back → shoulders.** Bodybuilding shoulder-day convention; scap-elevation/shrug kinematics align with delts more than spinal stabilizers. `lowerTraps` stays in back. Tag assignments on individual exercises unchanged — only `MUSCLE_TO_GROUP['upperTraps']` flipped. `seed-tagging-principles.md` Group-specific conventions (Back + Shoulders) updated inline. Affects: picker Step 2 muscle-chip section, group chip filter inclusion, future F27/F24 stat roll-ups. No re-curation required. |
