...
===========================
Last updated: 2026-04-26 (session 53 CLOSED — punch-list triage + smoke pass complete. **13 punch-list items closed:** 6 shipped (#6 Hyperextension rename, #13 Deficit RDL added as variant, #15 Staggered-Stance RDL rename, #7 autoCapitalize sentences on picker custom-create, #12 variant rows hide muscle meta + chevron 22→28px+700, #17 primary-only muscle search + #18 hide secondaries on parent rows + EMG future direction note); 7 deferred (#1→F42 bodyweight redesign, #9→F40/F32 olympic-lift hide, #10→F40 Row C drill-down chips with 4 lock-ins, #11 cardio→multi-shape tracking arch dependency, #19→absorbed into EMG cycle, #8→memory project_log_datetime_edit). **2 findings logged as CP2/CP3** (exercise-bank.md group-label drift audit + seed-tagging-principles.md mid-trap clarification). **Mid-smoke gap fix:** picker had a duplicate local copy of `matchesMuscleTag` not updated by the service-layer #17 fix — patched. **F8 superseded by F42** (bodyweight + added-resistance redesign covers display AND stats honesty; F8's narrower `isBodyweight` boolean patch absorbed). **3 new memory entries:** project_bodyweight_added_resistance.md, project_emg_top_n_display.md, project_log_datetime_edit.md. **3 new feedback memories from session corrections:** feedback_ask_dont_lean.md, feedback_question_format.md, plus updates to existing memories. **Manual smoke pass results:** library renames pass post-IndexedDB-clear; row-rendering passes (parent primary-only, variants name-only); search behavior pass post-HMR-restart (biceps→curls + Narrow-Grip Pull-Up via Rule 1 override; glutes excludes Squat); regression checks all pass. **F40 Row C lock-ins added** (4 specs: state cleanup, any-primary filter, conditional visibility, 3-row UX cost flag). Files changed (5 src/, 4 artifacts): seed.ts (Hyperextension rename + Deficit RDL add + B-Stance→Staggered-Stance rename), ExerciseService.ts (matchesMuscleTag primary-only), ExerciseService.test.ts (test rewrite asserts negative), ExerciseSearchModal.tsx (renderMuscleMeta drop secondaries + matchesMuscleTag primary-only + autoCapitalize sentences), ExerciseSearchModal.module.css (chevron 28px/700), exercise-bank.md (3 row syncs), master-schematics.md (F40+F42 added/annotated, F41 closes-by extended, F38 dependency lock added, CP2+CP3 added, F8 → resolved), handoff.md (Session 53 punch-list closures inline), recap.md (this entry). Verification PENDING — user to run `npm run build && npx vitest run` (shell can't reach node from agent). Session 53 CLOSED. Previously: session 52 CLOSED — partial Step 7 of 7. Vitest coverage shipped (26 ExerciseSearchModal cases, 130/130 across 11 files). **Decisions #30 + #31 + #32 locked + SetRow stepper rebuild + F40/F41 logged.** #32 = `bias` → `modifications` schema rename + multi-select reshape (`string[]` default `[]`); value enum extended with `deficit` to 7; word `bias` reserved for a future muscle-loading-emphasis field (deferred to next cycle / F39). Decision #30: `MUSCLE_TO_GROUP['upperTraps']` flipped back → shoulders (lowerTraps stays in back). Decision #31: Farmer + Suitcase Carry primary order swap to `['upperTraps', 'forearms']` (routes both to Shoulders chip alongside shrugs). SetRow rebuilt with custom `[− input +]` pill stepper per cell — native browser spinner hidden via CSS, grid retuned (BEST 44px, LB `minmax(72px, 1fr)`, REPS 56px, RPE 76px, gap 2px, buttons 18×36); ExerciseCard column-header grid synced to same template so labels stay over inputs; save logic split into `saveWeight` / `saveReps` / `saveRpe` (blank field never fail-validates an adjacent field's save). Closes Session 53 punch-list #2 + #3. F40 (picker filter revamp) UX layout locked to 3-row design + v4 bundling note added; F41 (per-exercise `aliases: string[]` v4 schema bump) added — closes-by absorbs Session 53 punch-list #4/#5/#14. Manual smoke + Issue Tracker scrub + remaining ~14 punch-list items deferred to Session 53. Verification: tsc clean, vite build clean, 130/130 tests across 11 files.) Previously: session 51 CLOSED — CE1/CE2 v3 BUILD STEP 6 of 7 complete: ExerciseSearchModal full rewrite per Decision #27 + CE2 EB2/EB5. **Decision #27 amended this session:** four-state tap cycle (neutral → primary → synergist → stabilizer → neutral) replaces original D6.2 two-tap + D6.3 long-press scheme; long-press removed entirely. Save-with-no-primary blocks with inline error rather than auto-promote. **Files changed (2 src/):** `src/components/ExerciseSearchModal.tsx` (~360 lines, full rewrite — three view modes [browse | createStep1 | createStep2]; browse renders flat in search mode and parents-with-chevron in browse mode per CE2 #5; chevron toggles inline variant expansion via `expandedParentIds: Set<number>` + `variantsByParentId` memo; custom rows show 🗑 trash with delete flow → reuses `Modal` for 0-variant case + bespoke radio choice modal for ≥1-variant case [default = null-orphan, safer]; sticky `+ Create custom exercise` footer always visible; Step 1 = name + multi-select group chips + optional parent picker `<select>` populated from parent-level exercises; Step 2 = sectioned muscle chips with 4-state tap cycle via `cycleRole`; back from Step 2 preserves Step 1 state and drops role tags for unticked groups), `src/components/ExerciseSearchModal.module.css` (~270 lines, full rewrite — kept old `.backdrop/.drawer/.searchInput/.chips/.chip/.chipActive/.list`; new `.rowGroup/.row/.rowMain/.rowActions` row layout; `.muscleMeta/.musclePrimary/.muscleSecondary` for browse-row metadata; `.variantRow + .variantPrefix` for indented `↳` variants; `.chevronBtn + .chevronExpanded` 90° rotation; `.trashBtn` 44×44; `.createFooterBtn` sticky bottom; `.formHeader/.formBackBtn/.formTitle/.fieldLabel/.fieldHint + 3 hint color spans/.formInput/.formInputError/.formSelect/.fieldError/.formPrimaryBtn:disabled`; Step 2 `.muscleSections/.muscleSection/.sectionHeader/.muscleChipRow/.muscleChip` + 4 role classes [`.muscleChipNeutral/Primary/Synergist/Stabilizer` — placeholder palette, designer to swap]; delete choice modal `.deleteBackdrop` z-index 300/`.deleteCard/.deleteTitle/.deleteBody/.deleteOption + .deleteOptionHint/.deleteActions/.deleteBtnSecondary/.deleteBtnDestructive` with `accent-color: var(--color-green)` on radio). **No service edits** — `ExerciseService.search/create/deleteExercise` contracts already shipped Session 49; variant lookup uses in-memory `allExercises` (no `getVariants` service method needed). **No test edits** — modal has no existing test file. **Verification:** `npx tsc --noEmit` clean; `npm run build` clean (645ms, 1694 modules); `npx vitest run` 104/104 tests across 10 files (unchanged). **Session 51 CLOSED.**)

CURRENT TASK (mirrors CLAUDE.md — if they diverge, recap.md wins)
-------------------------------------------------------------------
Phase: 5 — CE1/CE2 v3 build (Step 7 of 7 effectively complete; full session-by-session walk through final smoke + punch-list closure done in Session 53)
Last session ended: Session 53 CLOSED 2026-04-26. **Punch-list triage + manual smoke pass complete.** 13 items processed: 6 shipped (#6 Hyperextension rename → "Back Hyperextension" + future parent slot noted; #13 Deficit RDL added as variant of Romanian Deadlift with `modifications: ['lengthened']` per principles enum; #15 B-Stance RDL renamed to Staggered-Stance RDL + F41 alias `["b-stance", "b stance"]` logged; #7 autoCapitalize "sentences" added to picker custom-create input only — broader sweep deferred; #12 variant rows now render `↳ [name]` with no muscle meta + chevron glyph bumped 22→28px / weight 700 for tap-target perception; #17 muscle text search now primary-only — service + duplicate picker copy both fixed + test rewritten as negative assertion; #18 picker rows hide secondaries — primary muscles only). 7 deferred (#1 → F42 bodyweight + added-resistance redesign, full v4 schema work; #9 → F40/F32 olympic-lift visibility; #10 → F40 Row C drill-down chips with 4 lock-ins annotated to F40; #11 → multi-shape tracking-arch dependency lock on F38; #19 → absorbed into EMG cycle, no standalone F-row; #8 → memory-only `project_log_datetime_edit.md`). 2 findings logged as cleanup items (CP2 exercise-bank.md group-label drift, CP3 seed-tagging-principles.md mid-trap clarification note). **F8 superseded by F42** — moved to Resolved Issues; F42 covers display + stats honesty whereas F8 was just `isBodyweight` boolean patch. **Mid-smoke gap fix:** picker had a duplicate local copy of `matchesMuscleTag` that was not updated by the service-layer fix — patched (refactor to shared helper deferred). **3 new project memories:** project_bodyweight_added_resistance.md (F42), project_emg_top_n_display.md (Phase 7+ direction with #19 absorption note), project_log_datetime_edit.md (#8). **2 new feedback memories from session corrections:** feedback_ask_dont_lean.md ("don't lead with 'my lean: X'" — biases user choice), feedback_question_format.md ("context + concrete tap-by-tap outcomes + plain English on every question — no jargon like 'affordance' or 'semantics'"). **Manual smoke results:** all 4 library renames pass post-IndexedDB-clear; row rendering passes (parent rows show only primary muscles, variant rows show name only); search behavior passes (biceps → curls + Narrow-Grip Pull-Up via Rule 1 override; glutes excludes Squat); regression checks pass (custom-create, trash/delete, SetRow stepper, RPE column). **Files changed (5 src/, 4 artifacts):** seed.ts (Hyperextension + Deficit RDL + Staggered-Stance RDL), ExerciseService.ts (matchesMuscleTag primary-only + JSDoc), ExerciseService.test.ts (negative assertion test), ExerciseSearchModal.tsx (renderMuscleMeta drop secondaries + EMG-future inline comment, matchesMuscleTag primary-only mirror, autoCapitalize sentences on custom-create input), ExerciseSearchModal.module.css (chevron font-size 28px + font-weight 700), exercise-bank.md (3 row syncs), master-schematics.md (F40 Row C lock-ins added; F41 closes-by extended; F42 added; F38 dependency lock added; CP2 + CP3 added; F8 → Resolved Issues), handoff.md (full Session 53 punch-list closures inline), recap.md (this entry). **Verification:** PENDING — agent shell can't run npx; user to run `npm run build && npx vitest run` before commit; expected build clean + 130/130 tests (#17 test rewrite changed assertion direction not count). **Build Step 7 of 7 effectively COMPLETE** — Session 53 closes the smoke + punch-list portion. Previously: Session 52 CLOSED 2026-04-26. **Build Step 7 of 7 partial — Vitest coverage shipped + 3 mid-session decisions (#30, #31, #32).** **Decision #32 (latest):** `Exercise.bias: string \| null` renamed and reshaped to `Exercise.modifications: string[]` (default `[]`). Multi-select. Value enum extended with `deficit` to 7 (`paused / tempo / partial / lengthened / peak-contraction / explosive / deficit`). Word `bias` reserved for a future field capturing which muscle takes more load as a result of a modification — deferred to next dev cycle (F39). 10 code files renamed + 3 spec files + 3 memory files updated. Pre-launch nuke-and-reseed (Decision #28) means no DB migration needed. Verified: tsc clean, build clean (2.89s), 130/130 tests across 11 files. Naming chosen to avoid collision with `parentExerciseId` (which already defines variant relationships); `modifications` is plural per multi-select convention. **Earlier Session 52 work:** (a) Vitest coverage for ExerciseSearchModal: 26 new cases across 4 describe blocks (browse / Step 1 / Step 2 / delete flow) — full suite 130/130 across 11 files (was 104/104 across 10). (b) **Decision #30 locked mid-session — `upperTraps` group derivation back → shoulders.** `MUSCLE_TO_GROUP['upperTraps']` flipped from `'back'` to `'shoulders'` in [src/db/muscleTaxonomy.ts](../src/db/muscleTaxonomy.ts:46); `lowerTraps` stays in back (scap retraction + depression with rhomboids/mid traps). Bodybuilding shoulder-day convention; scap-elevation kinematics align with delts. Tag assignments on individual exercises unchanged — only broad-group derivation moved. Effects: picker Step 2 muscle-chip section moves upperTraps from Back to Shoulders; "Shoulders" chip filter now includes any exercise with upperTraps as first primary; future F27/F24 stat roll-ups count upperTraps under shoulders. (c) **Decision #31 locked mid-session — Farmer Carry + Suitcase Carry primary order swap.** Both reordered from `['forearms', 'upperTraps']` → `['upperTraps', 'forearms']` ([seed.ts:603, :606](../src/db/seed.ts#L603)). Routes both to Shoulders chip alongside shrugs (`getExerciseGroup()` uses `primaryMuscles[0]`). Heavy-carry trap stimulus dominates over forearm grip in training adaptation; co-primary tagging preserved. Other carries unaffected (Overhead → Shoulders ✅, Zercher → Back ✅). Curator flag on #31 row: Suitcase Carry obliques should be co-primary (anti-lateral flexion is its unique stimulus vs Farmer) — deferred to future tagging audit, logged in `memory/project_suitcase_carry_obliques_flag.md`. (d) Test file typing fix: `src/components/ExerciseSearchModal.test.tsx` lines 56–57 + import — `let onSelect: ReturnType<typeof vi.fn>` → `let onSelect: Mock<(exercise: Exercise) => void>` (with `Mock` import from vitest). Pre-existing issue from Session 52 partial commit (d989d05) — runtime tests passed but `tsc -b` build mode failed. Surgical typing-only fix to unblock verification; no test logic changed. (e) Spec sync: `master-schematics.md` Decision #31 row appended (after #30); changelog entry added; `exercise-bank.md` rows 32 + 33 augmented with chip-routing notes. **Verification:** `npx tsc --noEmit` clean; `npm run build` clean (781ms, 1694 modules); `npx vitest run` 130/130 tests across 11 files. Manual smoke + Issue Tracker scrub for CE1+CE2 + commit still pending. Previously: Session 51 CLOSED 2026-04-26. **Build Step 6 of 7 complete (ExerciseSearchModal rewrite).** **Decision #27 amended:** four-state tap cycle (neutral → primary → synergist → stabilizer → neutral) replaces D6.2 + D6.3; save-with-no-primary blocks with inline error. **Files changed (2 src/):** `src/components/ExerciseSearchModal.tsx` (full rewrite, ~360 lines — three view modes; browse-mode chevron expansion + search-mode flat per CE2 #5; sticky create footer; Step 1 name + multi-select group chips + optional parent picker; Step 2 sectioned muscle chips with 4-state tap cycle; trash on custom rows; bespoke radio choice modal for ≥1-variant delete + reuses `Modal` for 0-variant case), `src/components/ExerciseSearchModal.module.css` (full rewrite, ~270 lines — kept old browse classes; added rowGroup/chevron/trash/variant/sticky-footer/two form steps/muscle role chips [4 placeholder classes]/delete choice modal). **Spec sync:** `master-schematics.md` Decision #27 row + ExerciseSearchModal Spec section updated inline. **No service edits** (contracts locked Session 49). **No test edits** (no existing modal test). **Verification:** `npx tsc --noEmit` clean; `npm run build` clean (645ms, 1694 modules); `npx vitest run` 104/104 tests across 10 files (unchanged). Previously: Session 50 CLOSED 2026-04-26. **Build Steps 4 + 5 of 7 complete (RPE plumbing).** **Files changed (7):** `src/context/UserSettingsContext.tsx` (added `rpeEnabled: boolean` to interface + memo, derived from `user?.rpeEnabled ?? false`; memo dep array now `[user?.unitPreference, user?.rpeEnabled]`); `src/pages/ProfilePage/index.tsx` (new `handleRpeToggle(next)` calls `UserService.updateProfile({ rpeEnabled })` then `updateUser({ rpeEnabled })`; new "RPE Tracking" Off/On toggle in the Preferences section, matches existing Imperial/Metric pattern); `src/pages/WorkoutDetailPage/index.tsx` (reads `rpeEnabled` from useUserSettings; `handleSetUpdate` now accepts `data: { weight?, reps?, rpe? }` and spreads into `LogSetService.update` + state setter; passes `rpeEnabled` to ExerciseCard); `src/pages/WorkoutDetailPage/components/ExerciseCard.tsx` (accepts + forwards `rpeEnabled`; conditionally renders RPE column header; `onSetUpdate` signature now partial); `src/pages/WorkoutDetailPage/components/ExerciseCard.module.css` (`.colHeadersWithRpe` 6-col grid override + `.colRpe` text-align); `src/pages/WorkoutDetailPage/components/SetRow.tsx` (accepts `rpeEnabled`; new `rpeStr`/`rpeError` state; new `saveRpe` validator — blank→null clear, otherwise 1–10 with `Number.isInteger(n*2)` half-step check; conditional RPE input with `min=1 max=10 step=0.5` + spinner-immediate-save matching weight/reps pattern; readOnly mode shows RPE static value or "—"); `src/pages/WorkoutDetailPage/components/SetRow.module.css` (`.rowWithRpe` 6-col grid override). **Public contract change:** SetRow.onUpdate / ExerciseCard.onSetUpdate switched from `(setId, weightLb, reps)` to `(setId, data: { weight?, reps?, rpe? })` so RPE saves independently and "null never blocks save" (Decision #26). **Verification:** `npx tsc --noEmit` clean; `npm run build` clean (426ms, 1694 modules); `npx vitest run` 104/104 tests across 10 files (unchanged — existing useUserSettings mock returns no `rpeEnabled` → undefined → falsy → RPE column hidden in test renders, no test edits needed). Previously: Session 49 CLOSED 2026-04-25 — Build Step 3 of 7 (service layer rebuild + P1 curation gap closed inline; library 212→214; 104/104 tests). **Files changed (5):** `src/services/ExerciseService.ts` (rewrite — `getAll` + `search(query, group?)` matching name + muscle id + MUSCLE_LABELS with background-muscle filter per D6.4 + group via getExerciseGroup; `create(exercise)` validates Rule 1 (rejects nesting under a variant) + Rule 3 (self-ref impossible at create) + non-blank name + ≥1 primary muscle; `deleteExercise(id, { cascade })` custom-only with cascade=true purging children + log/template history vs cascade=false null-orphaning children; private `purgeExerciseRecord` uses .filter() because exerciseId is not indexed on logExercises/workoutExercises — acceptable at MVP scale, runs only on rare custom-delete path), `src/services/LogSetService.ts` (`add(logExerciseId, rpe?)` writes through; `update(id, { weight, reps, rpe })` adds strict RPE validation: 1–10 in 0.5 increments OR null), `src/services/UserService.ts` (updateProfile Pick extends to rpeEnabled + trainingAge), `src/services/ExerciseService.test.ts` (NEW — 22 cases), `src/services/LogSetService.test.ts` (+6 rpe cases). **P1 gap closed inline:** added 2 entries to seed-draft.md + seed.ts: Machine Row (P1 #9, parent-level, inherits Iso-Lat Low Row map — `[lats, upperBack]` co-primary) + Neutral-Grip Pull-Up (P1 #8, structural variant of Pull-Up with parent-map inheritance — no Rule 1 override since neutral grip not in EMG override queue). Library 212→214; 28 variant FKs; SEED_ENTRIES.length now matches exercise-bank.md tier counts (P1=56, total=214 — no exercise-bank edits needed). **Verification:** `npx tsc --noEmit` clean; `npm run build` clean (2.34s, 1694 modules); `npx vitest run` 104/104 tests across 10 files (was 76/9; +28 tests, +1 file). Previously: Session 48 CLOSED 2026-04-25 — Build Step 2 of 7 (seed library compiled, 212-entry catalog from seed-draft.md → typed-Exercise SEED_ENTRIES + 2-pass seedExercises). Previously: Session 47 CLOSED 2026-04-25. **Build Step 1 of 7 complete (foundation layer).** First code-touching session of the CE1/CE2 v3 build cycle. **Decisions locked at session start:** (1) Dexie in-code version number = `3` (matches planning-doc labels rather than code's natural `version(2)` progression — `version(1)` retained alongside, `version(2)` skipped since no v2 was ever shipped); (2) Pending v2-era schema additions (`bodyMetrics`, `dailyCheckins`, `workoutLogs.rating`, `users.goalWeight`) stay deferred — bundle into the schema bump when their features actually ship, not now; (3) Pacing = Option A (4 sessions originally; revised mid-execution to ~5 after seed-draft.md size assessment). **Files touched (8):** `src/types/index.ts` (new Muscle/MuscleGroup/SecondaryRole/SecondaryMuscle types; rewrote Exercise dropping `category` and adding `parentExerciseId` + `primaryMuscles` + `secondaryMuscles` + 6 Tier 3 forward-compat; added User.rpeEnabled + User.trainingAge; added LogSet.rpe), `src/db/muscleTaxonomy.ts` (NEW: MUSCLE_LABELS / MUSCLE_TO_GROUP / RECOVERY_WINDOWS / SECONDARY_VOLUME_MULTIPLIER / MUSCLE_GROUPS / MUSCLE_GROUP_LABELS / getExerciseGroup), `src/db/db.ts` (version(3) declared with v3 schema string + .upgrade() hook clearing exercises/logExercises/logSets per D8.1 sub-option B2), `src/db/seed.ts` (stub — Step 2 fills in 214 entries), `src/services/ExerciseService.ts` (trimmed to `getAll()` only — Step 3 reintroduces search/create/deleteExercise with new shape), `src/services/AuthService.ts` (signup populates rpeEnabled: false + trainingAge: null), `src/components/ExerciseSearchModal.tsx` (chip filter switched to MuscleGroup via getExerciseGroup; create form stripped — Step 6 rebuilds full Decision #27 picker with chevron/two-step/parent-picker/deletion-modal), 4 test files patched (LogSetService.test, WorkoutExerciseService.test, WorkoutLogService.test, WorkoutService.test) to construct Exercise with new shape. **Verification:** `npx tsc --noEmit` clean; `npm run build` clean (6.99s, 1694 modules); `npx vitest run` 72/72 tests passing across 8 files. **Build Steps 2–7 still ahead** (seed compile, services, RPE plumbing, modal rewrite, smoke + close). **Pre-build batch (8 items) + Cross-conversation safety batch (3 items) = 11 gap-audit items closed.** Pre-edit audit revealed 7 of 8 Pre-build items already resolved inline during sessions 41–44 spec patching (gap-audit was authored before some of those sessions completed; fixes happened along the way but the audit doc was never updated to reflect that). **Already-resolved (no edit, audit-log only):** GA-02 (`ExerciseService.create()` already includes `parentExerciseId` per session 44 EB5 integration); GA-17 (`logs.md:227` `getByWorkoutLogId`); GA-18 (`statistics.md:374` `getNeglectedGroups`); GA-19 (Overview Dashboard table — no adherence row); GA-20 (`getAdherenceRate` not present in either service table); GA-44 (CE2 memory `project_ce2_schema_architecture.md:68` already drops `category`); GA-45 (CE2 memory already points to master-schematics § ExerciseSearchModal Spec). **Newly fixed this session:** **GA-01** — added explicit v3 Dexie schema code block alongside v2 in `master-schematics.md` § Dexie Schema String; v3 string adds `parentExerciseId` as secondary index on exercises; concurrent close-out of **GA-13** (mislabeled "v3 lock Session 41" header now corrected). **GA-42** — rewrote `project_state.md` body to post-45g state (re-curation COMPLETE 214/214; six-coach panel methodology + accuracy lens documented; library kinetic-chain ceiling noted; Session 47+ CE1/CE2 build queued). **GA-43** — refreshed MEMORY.md project_state index line. **GA-46** — restructured `project_artifact_fixes.md`: 4 of 6 sub-items resolved (page title done s30; FAB states done s30; color-preview moot; CLAUDE.md Post-Demo Cleanup section deleted s28); remaining 2 (FAB size 56→52px, FAB position) re-pointed at GA-40 + GA-41 as canonical trackers. Gap-audit.md status header bumped: 13/51 closed; 38 hygiene+cosmetic remain (deferrable; can roll out as artifacts are touched). **Session 46 CLOSED.** Previously: Session 45g CLOSED 2026-04-24 — 12 parked OL + KB-ballistic entries tagged; seed re-curation COMPLETE 214/214; new "Olympic lift" template codified; six-coach panel expanded with Poliquin; tagging accuracy lens formalized per `feedback_tagging_accuracy.md`; NEW library kinetic-chain ceiling (C&J 13, Snatch 12). **12 previously-parked P4 entries tagged in `artifacts/seed-draft.md` § Session 45g § Full entry blocks**: Group A Cleans (Power Clean, Hang Clean, Clean & Jerk) + Group B Snatches (Power Snatch, Hang Snatch) + Group C Pulls (Clean Pull, Snatch Pull) + Group D Jerks/Push Press (Push Jerk, Split Jerk, Push Press) + 2 KB-ballistic (KB Clean, KB Snatch). **Queue cleared — seed re-curation COMPLETE (214/214 entries tagged across sessions 45a–g).** **New movement-pattern template codified** in principles doc: "Olympic lift / ballistic triple extension" with 3 sub-patterns (Pulling = Hinge base + catch additions; Jerk = Vertical push + leg drive; KB-ballistic = KB Swing base + catch additions). **Six-coach consult panel expanded:** Poliquin formally added alongside Schoenfeld / Haff / Tsatsouline / Dan John / Nuckols (Poliquin lens drove snatch-grip upperBack syn promotion + cuff stab on overhead heavy free-path). **Tagging accuracy lens formally codified** per `feedback_tagging_accuracy.md` (saved 2026-04-24 outside repo per CLAUDE.md auto-memory): correct-execution + common-mistake recruitment scope (e.g., bar drift on OL pull = lats bail-out; bar drift on jerk dip = upperBack bail-out). NOT extreme edge cases. Drove broad lats-on-OL-pulls + upperBack-stab-on-jerks + calves-on-KB-Clean + sideDelts-on-KB-Snatch additions. **NEW LIBRARY KINETIC-CHAIN CEILING:** Clean & Jerk at 13 secondaries (supersedes Overhead Carry at 10); Power/Hang Snatch at 12 (snatch-family ceiling). Defensible per six-coach consensus — OL lifts are categorically the most kinetic-chain-demanding exercises in library by design. **4 new EMG overrides** added to principles-doc reference list: Olympic Cleans + Snatches + Jerks + KB-ballistic (each with sub-pattern rationale). **12 exceptions logged + all template-driven** under new `### Session 45g exceptions` subsection. **Cross-curation validation pass executed alongside**: 16 existing overrides re-verified valid; 4 new overrides added; 0 new FK linkages (all 12 standalone); group-derivation spot checks clean across full 120-entry curation corpus. Zero code, zero src/ edits, zero memory-file edits except `feedback_tagging_accuracy.md` creation + MEMORY.md index update. **Session 45g CLOSED.**
Next action: **Session 54 = TBD scope.** Session 53 closes the CE1/CE2 v3 build cycle in practical terms — Step 7 (smoke + Issue Tracker close-out) complete. Candidate scopes for Session 54: (a) Phase 6 kickoff — pick the next major build (Stats / F32 toggle menu / EMG planning — depends on user priority); (b) Cleanup pass on CP1/CP2/CP3 + remaining gap-audit items; (c) F40 Row C build (Phase 6+ — picker filter revamp with the 4 lock-ins from Session 53); (d) F41 build (per-exercise aliases — closes punch-list #4/#5/#14/#15); (e) F42 build (bodyweight + added-resistance redesign — closes #1). **Verification still owed for Session 53:** user to run `npm run build && npx vitest run` and confirm clean before commit.

Required reading next session: recap.md (this file — Session 53 entry above), handoff.md (top of file — Session 53 punch-list closures + Session 52 entry), `artifacts/master-schematics.md` Issue Tracker (F40-F42 + CP1-CP3 newly added/annotated; F8 → Resolved). Open relevant memory entries via MEMORY.md index. Do NOT re-read seed-draft / muscleTaxonomy / service files unless next session's scope touches those areas.
...
Most recent at top. Full history in handoff.md.
  ...
  Session 47 — 2026-04-25 (CE1/CE2 V3 BUILD STEP 1 of 7 CLOSED — foundation layer landed; tsc/build/72 tests all green)
    - First code-touching session of the CE1/CE2 v3 build cycle.
    - **Decisions locked at session start:**
        - **Dexie in-code version = `3`** (not `2`). Code originally at `version(1)` only — never had a `version(2)` declaration. Picked `3` to match planning-doc labels (master-schematics § Dexie Schema String — v3). `version(1)` retained alongside; `version(2)` skipped (no v2 was ever shipped to anyone).
        - **Pending v2-era schema additions stay deferred:** `bodyMetrics` table, `dailyCheckins` table, `workoutLogs.rating`, `users.goalWeight` were specced in master-schematics line 326 as "bundle into the same v3 hop" but none exist in `src/db/db.ts`. Defer until the relevant feature ships, rather than adding no-op forward-compat columns now.
        - **Pacing = Option A (4 sessions, revised mid-execution to ~5).** Original plan put Steps 1+2+3 in Session 47; reading `seed-draft.md` size up close (3,676 lines × 214 entries × ~17 lines of structured content each) revealed Step 2 alone deserves its own focused session. Stopped after Step 1 per build rule "Prefer finishing the current file/component cleanly over starting a new one."
    - **Files changed (8):**
        ...
        - **`src/db/muscleTaxonomy.ts`** *(NEW)* — single source of truth for muscle taxonomy: `MUSCLE_LABELS` (Title Case display), `MUSCLE_TO_GROUP` (specific → broad with neck → shoulders), `RECOVERY_WINDOWS` (Decision #25 — large 60h × 9 / small 36h × 15 / background-small 36h × 2), `SECONDARY_VOLUME_MULTIPLIER = 0.5`, `MUSCLE_GROUPS` readonly array, `MUSCLE_GROUP_LABELS`, `getExerciseGroup()` helper (first-primary-wins tiebreak per Decision #28).
        ...
        - **`src/db/seed.ts`** — stubbed: keeps `seedExercises()` export so App.tsx still compiles, but body is a no-op until Step 2 fills in 214-entry compilation. Picker will show empty list until Step 2 lands.
        - **`src/services/ExerciseService.ts`** — trimmed to `getAll()`. Removed `search(query, category)` (unused) and `create(name, category)` (only called from the stripped modal create form). Step 3 reintroduces all three with new signatures (search matches name + muscle tags per D7; create accepts new schema fields + parentExerciseId per CE2; new `deleteExercise(id, { cascade })` for choice modal).
        ...
        - **`src/components/ExerciseSearchModal.tsx`** — chip filter switched from stored `Exercise['category']` to derived `MuscleGroup` via `getExerciseGroup()`. Inline custom-create form stripped entirely (state, handlers, JSX). Step 6 rebuilds the full Decision #27 picker (variant chevron expand/collapse, name+muscle-tag search, two-step custom create with optional parent picker, deletion choice modal).
    ...
    - **Plan vs reality:** original Option A pacing put Steps 1+2+3 in this session. Stopped after Step 1 per the "raise plan changes mid-execution" rule. Pacing now ~5 sessions: 47 (Step 1 — done) → 48 (Step 2 seed compile) → 49 (Step 3 services) → 50 (Steps 4+5 RPE) → 51 (Step 6 modal rewrite) → optional 52 or fold into 51 (Step 7 smoke + close-out).
    - **Carry-forward for Session 48:** required reading is recap.md (this entry), `artifacts/seed-draft.md` (compilation source), `artifacts/seed-tagging-principles.md`, `src/types/index.ts` (target shape), `src/db/muscleTaxonomy.ts` (Muscle union to verify against). Do NOT re-read tab specs / master-schematics / CE1+CE2 memory.
    - Zero memory-file edits. Session 47 CLOSED.

  Session 46 — 2026-04-24 (PRE-BUILD GAP-AUDIT BATCH CLOSED — 11 items resolved; 7 already-done audit-only + 4 newly fixed; ready for Session 47+ CE1/CE2 build)
    - Research + artifact mode. No code, no `src/` edits.
    - **Pre-edit audit revealed substantial scope reduction:** 7 of 8 Pre-build items already resolved inline during sessions 41–44 spec patching. Gap-audit doc was authored before some of those sessions completed; the work happened along the way but the audit was never updated to reflect closures.
    - **Already-resolved (no edit, audit-log only):**
        - GA-02 — `ExerciseService.create()` already includes `parentExerciseId` at master-schematics line 364 (set during session 44 EB5 integration)
        - GA-17 — `logs.md:227` already shows `getByWorkoutLogId` (fixed in earlier rename pass)
        - GA-18 — `statistics.md:374` already shows `getNeglectedGroups` (fixed during session 42 CE1 spec patch 2/3)
        - GA-19 — `statistics.md` Overview Dashboard table (lines 139-145) has no adherence row
        - GA-20 — `getAdherenceRate` not present in either statistics.md or master-schematics.md service tables
        - GA-44 — `project_ce2_schema_architecture.md:68` v3 schema example already drops `category`
        - GA-45 — `project_ce2_schema_architecture.md` lines 31, 204, 362 already point to master-schematics § ExerciseSearchModal Spec
    - **Newly fixed this session (4 items):**
        - **GA-01 (proper)** — added explicit v3 Dexie schema code block alongside v2 in `master-schematics.md` § Dexie Schema String. v3 string adds `parentExerciseId` as secondary index on exercises (`'++id, name, parentExerciseId'`). Diff-vs-v2 note explains why other v3 additive fields (muscles, RPE, Tier 3) are stored but not indexed at MVP scale. Concurrent close-out of **GA-13** (header label was misleading — block labeled "v3 lock Session 41" but showed v2 string; now both versions present with clear sub-headers).
        - **GA-42** — rewrote `project_state.md` body to post-45g state: re-curation COMPLETE (214/214 entries across 7 sub-sessions 45a–g), six-coach panel methodology + tagging accuracy lens documented, library kinetic-chain ceiling noted (C&J 13, Snatch 12), Session 47+ CE1/CE2 build queued.
        - **GA-43** — refreshed MEMORY.md project_state index line: "45a–e closed, 45f next" → "45a–g closed, 214/214 entries; Session 46 spec-patch hygiene in progress".
        - **GA-46** — restructured `project_artifact_fixes.md`: original 3-fix list audited per sub-point. 4-of-6 sub-items resolved (page title done s30; FAB states done s30; color-preview moot; CLAUDE.md Post-Demo Cleanup section deleted s28). Remaining 2 sub-items (FAB size 56→52px, FAB position bottom-right→inline center) re-pointed at GA-40 + GA-41 as canonical trackers; file flagged for deletion once GA-40/41 close.
    - **Gap-audit.md status header bumped:** 13/51 closed (Pre-45a 2 + Pre-build 8 + Cross-conversation safety 3); 38 hygiene+cosmetic remain (deferrable; can roll out as artifacts are touched in future sessions).
    - **Methodology note:** Audit-before-edit revealed the cleanest way to "execute a batch" is to first verify each item's current state against the audit's claim. The audit was a snapshot; specs evolve. Saved ~2 hr of redundant edits.
    - **Close-out:** 4 actual artifact edits + 1 audit log update + recap.md/CLAUDE.md sync. Memory edits: project_state.md, MEMORY.md index, project_artifact_fixes.md (3 memory files touched). Session 46 CLOSED.
